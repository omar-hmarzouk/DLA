 ...
 *******************************************************************************
 *
 * @sa MORSE_zunmqr_Tile
 * @sa MORSE_zunmqr_Tile_Async
 * @sa MORSE_cunmqr
 * @sa MORSE_dormqr
 * @sa MORSE_sormqr
 * @sa MORSE_zgeqrf
                  ...
                  MORSE_Complex64_t *C, int LDC)
{
    ...
    if (morse == NULL) {
        morse_fatal_error("MORSE_zunmqr", "MORSE not initialized");
    ...
    if ((side != MorseLeft) && (side != MorseRight)) {
        morse_error("MORSE_zunmqr", "illegal value of side");
        return -1;
    }
    if ((trans != MorseConjTrans) && (trans != MorseNoTrans)){
        morse_error("MORSE_zunmqr", "illegal value of trans");
        return -2;
    }
    if (M < 0) {
        morse_error("MORSE_zunmqr", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        morse_error("MORSE_zunmqr", "illegal value of N");
        return -4;
    }
    if ((K < 0) || (K > Am)) {
    ...
    if (status != MORSE_SUCCESS) {
        morse_error("MORSE_zunmqr", "morse_tune() failed");
        return status;
    }
 ...
 *
 *  MORSE_zunmqr_Tile - overwrites the general M-by-N matrix C with Q*C, where Q is an orthogonal
 ...
 *
 *  Non-blocking equivalent of MORSE_zunmqr_Tile().
                             ...
                             MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    ...
    if (sequence == NULL) {
        morse_fatal_error("MORSE_zunmqr_Tile", "NULL sequence");
        return MORSE_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        morse_fatal_error("MORSE_zunmqr_Tile", "NULL request");
        return MORSE_ERR_UNALLOCATED;
    }
    ...
    if (morse_desc_check(A) != MORSE_SUCCESS) {
        morse_error("MORSE_zunmqr_Tile", "invalid first descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(T) != MORSE_SUCCESS) {
        morse_error("MORSE_zunmqr_Tile", "invalid second descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if (morse_desc_check(C) != MORSE_SUCCESS) {
        morse_error("MORSE_zunmqr_Tile", "invalid third descriptor");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    /* Check input arguments */
    if (A->nb != A->mb || C->nb != C->mb) {
        morse_error("MORSE_zunmqr_Tile", "only square tiles supported");
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
    }
    if ((side != MorseLeft) && (side != MorseRight)) {
        return morse_request_fail(sequence, request, MORSE_ERR_ILLEGAL_VALUE);
