...
void *(*MORSE_realThStart)(void *);

/*
  This function is only called to have a proxy that is compatible with WINAPI.
 */
unsigned WINAPI MORSE_winThStart(void *arg) {
  MORSE_realThStart( arg );
  return 0;
}

MORSE_DLLPORT int MORSE_CDECL pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start) (void *), void *arg) {
...
MORSE_DLLPORT int MORSE_CDECL pthread_setconcurrency (int level) {
